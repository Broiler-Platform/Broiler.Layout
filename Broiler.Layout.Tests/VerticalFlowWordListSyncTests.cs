...
public sealed class VerticalFlowWordListSyncTests
{
    ...
    private sealed class MeasuringLayoutEnvironment : Broiler.Layout.ILayoutEnvironment
    {
        ...
        public SizeF MeasureText(Broiler.Graphics.Text.ILayoutFont font, string text) => new((float)(text.Length * CharWidth), 16);

        public void MeasureText(Broiler.Graphics.Text.ILayoutFont font, string text, double maxWidth, out int charFit, out double charFitWidth)
        {
            charFit = text.Length;
            charFitWidth = text.Length * CharWidth;
        }

        public double GetWhitespaceWidth(Broiler.Graphics.Text.ILayoutFont font) => CharWidth;
